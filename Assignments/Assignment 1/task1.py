...
def task1(review,stopwords,year,m,n):
    ...
    result['B'] = review.filter(lambda x: datetime.strptime(x['date'], '%Y-%m-%d %H:%M:%S').year == year).count()
    result['C'] = review.map(lambda x: x['business_id']).distinct().count()
    ...
    stopwords.extend(['',' '])
    result['E'] = review.flatMap(lambda x: clean(x['text'])).filter(lambda x: x.strip() not in stopwords)\
        .map(lambda x: (x,1)).reduceByKey(lambda a, b: a + b).sortBy(lambda x: -x[1]).map(lambda x: x[0]).take(n)
    return result
#%%
